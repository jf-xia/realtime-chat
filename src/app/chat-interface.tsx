...
const promptSearch = (text: string) => {
  if (!text) return promptArray.map(([title, content]) => ({
    id: Math.random(),
    title,
    content,
  } as Prompt));
  return promptArray
    .filter(
      ([title, content]) => (title.includes(text) || content.includes(text)))
    .map(([title, content]) => ({
      id: Math.random(),
      title,
      content,
  ...
  useEffect(() => {
    const onKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.key === "ArrowUp") {
        ...
        const selectedPrompt = props.prompts[selectIndex];
        if (selectedPrompt) {
    ...
    <div className="relative w-full max-h-96 overflow-y-auto bg-white border border-gray-300 rounded-lg z-50">
      {props.prompts.map((prompt, i) => (
        <div
          ...
          className={`p-4 cursor-pointer ${i === selectIndex ? "bg-gray-100" : ""
            }`}
        ...
        >
          ...
          <div className="text-gray-600 text-sm">{prompt.content}</div>
        </div>
      ))}
    </div>
  );
